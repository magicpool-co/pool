...
func NewTx(privKey ed25519.PrivateKey, networkID, fromAddress, toAddress string, value *big.Int, nonce uint64) (string, error) {
	tx := &transaction{
...
func HashTx(encodedTx []byte) string {
	return b58Encode(txHashPrefix, crypto.Blake2b256(encodedTx))
}
