...
func (suite *PooldbReadsSuite) TestReadMiner() {
	...
	_, err = pooldb.GetActiveMiners(pooldbClient.Reader(), 0, 0)
	if err != nil {
...
func (suite *PooldbReadsSuite) TestReadBalanceOutput() {
	...
	_, err = pooldb.GetUnpaidBalanceOutputsAboveThreshold(pooldbClient.Reader(), "ETH", "10")
	if err != nil {
		suite.T().Errorf("failed: GetUnpaidBalanceOutputsAboveThreshold: %v", err)
	}
}
